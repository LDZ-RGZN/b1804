...
def one():
    ...
    money = input ('请问您想要存款取款还是转账:')
    if money == '存款':
        mon = float (input ('请输入您要存款的金额：'))
        print ('您的%0.2f人民币以存入您的账户中，感谢您的使用'%mon)
    elif money == '转账':
        accounts = float (input('请输入您要转账的金额：'))
        account = input ('请输入您要转入的账户：')
        print ('您以成功的向%s账户转入%0.2f人民币'%(account,accounts))
    elif money == '取款':
        accountb = float (input('请输入您要取款的金额：'))
        print ('正在处理...')
        time.sleep(1)
        print ('交易成功')
